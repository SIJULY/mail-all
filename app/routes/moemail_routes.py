...
from app.repositories.mail_repo import ensure_managed_mailbox, get_managed_domains, get_managed_mailbox_by_id
from app.services.message_service import serialize_moemail_message
from app.services.settings_service import choose_moemail_domain
...
def register_moemail_routes(app):
    ...
    def moemail_config():
        ...
        domains = managed_domains if managed_domains else ["example.com"]
    ...
    def moemail_message_detail(email_id: str, message_id: str):
        ...
        try:
            row = conn.execute(
                "SELECT * FROM received_emails WHERE id = ? AND lower(trim(recipient)) = lower(trim(?))",
    ...
    def moemail_share_email(email_id: str):
        return jsonify({"success": True, "id": str(email_id), "url": f"/Mail?token={SPECIAL_VIEW_TOKEN}"})
