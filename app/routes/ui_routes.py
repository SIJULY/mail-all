...
def register_ui_routes(app):
    ...
    def view_drafts():
        ...
        try:
            rows = conn.execute(
                "SELECT * FROM draft_emails WHERE owner_email = ? ORDER BY updated_at DESC, id DESC",
                (session["user_email"],),
            ).fetchall()
        finally:
            conn.close()
    ...
    def view_sent():
        ...
        try:
            rows = conn.execute(
                "SELECT * FROM sent_emails WHERE owner_email = ? ORDER BY id DESC",
                (session["user_email"],),
            ).fetchall()
        finally:
            conn.close()
    ...
    def compose_email():
        from app.repositories.db import get_db_conn
        from app.services.smtp_service import get_smtp_config, send_email_via_smtp
        ...
        form_data = {"to": "", "subject": "", "body": "", "html_body": "", "editor_mode": "text"}
        ...
        if request.method == "POST":
            ...
            if editor_mode not in ("text", "html"):
                editor_mode = "text"
            form_data = {"to": to_address, "subject": subject, "body": body, "html_body": html_body, "editor_mode": editor_mode}
            ...
            if not smtp_cfg["password"] or not smtp_cfg["default_sender"]:
                ...
                success, message = send_email_via_smtp(to_address, subject, body, html_body)
                flash(message, "success" if success else "error")
                if success:
                    conn = get_db_conn()
                    try:
                        conn.execute(
                            "INSERT INTO sent_emails (owner_email, recipient, sender, subject, body, body_type) VALUES (?, ?, ?, ?, ?, ?)",
                            (
                                session["user_email"],
                                to_address,
