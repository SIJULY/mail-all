...
def register_admin_routes(app):
    ...
    def admin_domains():
        from app.routes.mail_routes import base_view_logic
        return base_view_logic(is_admin_view=True, nav_mode="domains")
    ...
    def admin_system():
        from app.routes.mail_routes import base_view_logic
        return base_view_logic(is_admin_view=True, nav_mode="system")
    ...
    def manage_smtp_settings():
        ...
        try:
            if send_mode == "resend":
                resend_token = request.form.get("resend_token") or ""
                if resend_token.strip():
                    set_app_setting("resend_token", resend_token)
            else:
    ...
    <div style="font-family:Arial,sans-serif;line-height:1.8;color:#111827;">
        <h2 style="margin:0 0 12px;">测试邮件</h2>
        <p>这是一封测试邮件。</p>
        <p><strong>发信模式：</strong>{html.escape(str(cfg.get('send_mode', 'smtp')))}</p>
        <p><strong>默认发件人：</strong>{html.escape(str(cfg.get('default_sender')))}</p>
    ...
    def manage_system_settings():
        ...
        try:
            if action == "tg":
                tg_bot_token = (request.form.get("tg_bot_token") or "").strip()
                tg_chat_id = (request.form.get("tg_chat_id") or "").strip()
                tg_sender_format = (request.form.get("tg_sender_format") or "name").strip()
                tg_recipient_format = (request.form.get("tg_recipient_display") or "show").strip()
                tg_enabled = "1" if request.form.get("tg_enabled") == "on" else "0"
                if tg_bot_token:
                    set_app_setting("tg_bot_token", tg_bot_token)
                set_app_setting("tg_chat_id", tg_chat_id)
                set_app_setting("tg_sender_format", tg_sender_format)
                set_app_setting("tg_recipient_format", tg_recipient_format)
                set_app_setting("tg_enabled", tg_enabled)
                flash("Telegram 通知配置已保存", "success")
            elif action == "refresh":
                ...
                flash("收件箱自动刷新配置已保存", "success")
            else:
                flash("未知的操作类型", "error")
        except Exception as e:
            app.logger.error(f"保存系统配置失败: {e}")
            flash(f"保存系统配置失败: {e}", "error")
        return redirect(url_for("admin_system"))

    @app.route("/send_test_tg_message", methods=["POST"])
    @login_required
    @admin_required
    def send_test_tg_message():
        from app.repositories.settings_repo import get_app_setting
        import requests

        tg_bot_token = get_app_setting("tg_bot_token", "").strip()
        tg_chat_id = get_app_setting("tg_chat_id", "").strip()

        if not tg_bot_token or not tg_chat_id:
            flash("请先填写并保存 Telegram Bot Token 和 Chat ID", "error")
            return redirect(url_for("admin_system"))

        url = f"https://api.telegram.org/bot{tg_bot_token}/sendMessage"
        payload = {
            "chat_id": tg_chat_id,
            "text": "这是一条测试消息。如果您收到此消息，说明 Telegram 推送配置正确！🎉"
        }
        
        try:
            response = requests.post(url, json=payload, timeout=10)
            if response.status_code == 200:
                flash("测试消息发送成功！请检查您的 Telegram。", "success")
            else:
                flash(f"测试消息发送失败: {response.text}", "error")
        except Exception as e:
