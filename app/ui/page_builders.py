...
from app.ui.sender_display import build_sender_display
...
def _build_sender_display(sender_raw: str):
    return build_sender_display(sender_raw, _SENDER_BRAND_MAP)
...
def render_email_list_page(
    ...
    if token_view_context:
        ...
        endpoint = (
            "view_trash"
            if nav_mode == "trash"
            else ("admin_view" if is_admin_view else "view_emails")
        )
        if nav_mode == "drafts":
            ...
            title_text = (
                f"回收站 ({session.get('user_email', '')} - 共 {total_emails} 封)"
            )
        else:
            title_text = (
                f"收件箱 ({session.get('user_email', '')} - 共 {total_emails} 封)"
            )
    ...
    if managed_domains is None:
        managed_domains = (
            get_managed_domains(include_inactive=True)
            if is_admin_view and not token_view_context
            else []
        )
    if managed_users is None:
        managed_users = (
            get_managed_users() if is_admin_view and not token_view_context else []
        )
    ...
    if smtp_modal_data is None:
        smtp_modal_data = {
            "server": smtp_cfg.get("server", ""),
            "port": smtp_cfg.get("port", ""),
            "username": smtp_cfg.get("username", ""),
            "default_sender": smtp_cfg.get("default_sender", ""),
            "password_configured": bool(smtp_cfg.get("password")),
            "test_recipient": "",
            "send_mode": smtp_cfg.get("send_mode", "smtp"),
            "resend_token": bool(smtp_cfg.get("resend_token")),
        }
    else:
        # 调用方（base_view_logic）传入的是原始配置，这里补齐模板所需的展示字段，
        # 且不把明文密码/Token 透传进模板上下文
        smtp_modal_data = {
            **smtp_modal_data,
            "password_configured": bool(smtp_modal_data.get("password")),
            "test_recipient": smtp_modal_data.get("test_recipient", ""),
            "resend_token": bool(smtp_modal_data.get("resend_token")),
        }
        smtp_modal_data.pop("password", None)
    ...
    filter_all_url = url_for(
        endpoint, **{**base_args, "filter": "all", "selected_id": None}
    )
    filter_read_url = url_for(
        endpoint, **{**base_args, "filter": "read", "selected_id": None}
    )
    filter_unread_url = url_for(
        endpoint, **{**base_args, "filter": "unread", "selected_id": None}
    )
    filter_code_url = url_for(
        endpoint, **{**base_args, "filter": "code", "selected_id": None}
    )
    filter_starred_url = url_for(
        endpoint, **{**base_args, "page": 1, "filter": "starred", "selected_id": None}
    )
    filter_important_url = url_for(
        endpoint, **{**base_args, "page": 1, "filter": "important", "selected_id": None}
    )

    home_url = url_for(endpoint, **{**base_args, "page": 1, "selected_id": None})
    prev_url = (
        url_for(endpoint, **{**base_args, "page": page - 1, "selected_id": None})
        if page > 1
        else None
    )
    next_url = (
    ...
    for item in emails_data:
        utc_dt = datetime.strptime(item["timestamp"], "%Y-%m-%d %H:%M:%S").replace(
            tzinfo=timezone.utc
        )
        bjt_str = utc_dt.astimezone(beijing_tz).strftime("%Y-%m-%d %H:%M:%S")
        body_for_preview = (
            strip_tags_for_preview(item["body"])
            if item["body_type"] and "html" in item["body_type"]
            else (item["body"] or "")
        )
        ...
        processed_emails.append(
            {
                ...
                "is_starred": (
                    ...
                    if hasattr(item, "keys") and ("is_starred" in item.keys())
                    else False
                ),
                "is_important": (
                    bool(item["is_important"])
                    if hasattr(item, "keys") and ("is_important" in item.keys())
                    else False
                ),
                "attachment_count": (
                    int(item["attachment_count"])
                    if hasattr(item, "keys") and ("attachment_count" in item.keys())
                    else 0
                ),
                "open_url": url_for(endpoint, **open_args),
                "toggle_star_url": url_for("toggle_star", email_id=item["id"]),
                "toggle_important_url": url_for(
                    "toggle_important", email_id=item["id"]
                ),
            }
        )
    ...
    if selected_email:
        utc_dt = datetime.strptime(
            selected_email["timestamp"], "%Y-%m-%d %H:%M:%S"
        ).replace(tzinfo=timezone.utc)
        bjt_str = utc_dt.astimezone(beijing_tz).strftime("%Y-%m-%d %H:%M:%S")
        body_content = selected_email["body"] or ""
        body_type = selected_email["body_type"] or "text/plain"
        has_sent_flag = hasattr(selected_email, "keys") and (
            "is_sent_mailbox" in selected_email.keys()
        )
        is_sent_detail = nav_mode == "sent" or (
            bool(selected_email["is_sent_mailbox"]) if has_sent_flag else False
        )
        _, reply_to_addr = parseaddr(selected_email["sender"] or "")
        can_reply = (
            False
            if is_sent_detail
            else bool(sending_enabled and reply_to_addr and "@" in reply_to_addr)
        )
        has_deleted_flag = hasattr(selected_email, "keys") and (
            "is_deleted" in selected_email.keys()
        )
        is_trash_detail = nav_mode == "trash" or (
            bool(selected_email["is_deleted"]) if has_deleted_flag else False
        )
        can_delete = (
            (not token_view_context)
            if is_sent_detail
            else (
                not token_view_context
                and can_delete_email(
                    selected_email,
                    is_admin_view,
                    token_view_context["mail"] if token_view_context else None,
                )
            )
        )
        if is_sent_detail:
        ...
        selected_email_data = {
            ...
            "text_body_html": linkify_plain_text(body_content),
            "can_reply": can_reply,
            "reply_url": url_for(
                "compose_email",
            ...
            "delete_label": (
                "删除已发送"
                if is_sent_detail
                else ("彻底删除" if is_trash_detail else "删除邮件")
            ),
            "delete_confirm": (
                "确定删除这封已发送邮件吗？"
                if is_sent_detail
            ...
            "delete_url": (
                url_for("delete_sent_email", sent_id=selected_email["id"])
                if is_sent_detail
                else (
                    url_for("permanently_delete_email", email_id=selected_email["id"])
                    if is_trash_detail
                    else url_for("delete_single_email", email_id=selected_email["id"])
                )
            ),
            "restore_url": url_for("restore_email", email_id=selected_email["id"]),
            "is_starred": (
                bool(selected_email["is_starred"])
                if hasattr(selected_email, "keys")
                and ("is_starred" in selected_email.keys())
                else bool(selected_email.get("is_starred", 0))
            ),
            "is_important": (
                bool(selected_email["is_important"])
                if hasattr(selected_email, "keys")
                and ("is_important" in selected_email.keys())
                else bool(selected_email.get("is_important", 0))
            ),
            "toggle_star_url": url_for("toggle_star", email_id=selected_email["id"]),
            "toggle_important_url": url_for(
                "toggle_important", email_id=selected_email["id"]
            ),
            "attachments": (
                selected_email.get("attachments", [])
                if hasattr(selected_email, "get")
                else []
            ),
        }
        for attachment in selected_email_data["attachments"]:
            attachment["download_url"] = url_for(
                "download_attachment", attachment_id=attachment["id"]
            )
...
function copyCode(e, code, emailId) {
    e.preventDefault();
    e.stopPropagation();
    ...
    if (navigator.clipboard && window.isSecureContext) {
        navigator.clipboard.writeText(code).then(showSuccess).catch(err => {
            ...
            fallbackCopyTextToClipboard(code, showSuccess);
        });
    } else {
        fallbackCopyTextToClipboard(code, showSuccess);
    }
...
function copyFieldValue(id){var field=document.getElementById(id);if(!field)return;var value=field.value||'';if(navigator.clipboard&&window.isSecureContext){navigator.clipboard.writeText(value);}else{field.select();document.execCommand('copy');field.setSelectionRange(0,0);}} function toggleAllCheckboxes(source,groupName){var selector=groupName?'input[data-group="'+groupName+'"]':'input[name="selected_ids"]';var checkboxes=document.querySelectorAll(selector);for(var i=0;i<checkboxes.length;i++){checkboxes[i].checked=source.checked;}} function closeDomainModalByMask(event){if(event.target&&event.target.id==='domain-modal'){closeDomainModal();}} function togglePlusAliasConfig(enabled){var baseEmail=document.getElementById('plus-alias-base-email');if(baseEmail){baseEmail.required=!!enabled; if(!enabled){baseEmail.value='';}}} function closeUserModalByMask(event){if(event.target&&event.target.id==='user-modal'){closeUserModal();}} function closeSmtpModalByMask(event){if(event.target&&event.target.id==='smtp-modal'){closeSmtpModal();}} function richCmd(cmd,value){if(value===null) return; document.execCommand(cmd,false,value||null); var editor=document.getElementById('rich_editor'); if(editor) editor.focus();} function syncRichEditorBeforeSubmit(){var editor=document.getElementById('rich_editor');var htmlBody=document.getElementById('html_body');if(editor&&htmlBody){htmlBody.value=editor.innerHTML;}} function autoRefreshInbox() {
    if(document.hidden) return;
    if(window.__mailAutoRefreshing) return;
    if(window.__isSearching) return; // Prevent interference with live search!
    
    var currentBody = document.getElementById('inbox-mail-tbody');
    if(!currentBody) return;
    
    window.__mailAutoRefreshing = true;
    ...
    fetch(currentUrl.toString(), {
        ...
        var newTitle = doc.querySelector('.mail-title');
        var currentTitle = document.querySelector('.mail-title');
        if (newTitle && currentTitle && newTitle.textContent !== currentTitle.textContent) {
            currentTitle.textContent = newTitle.textContent;
        }
        var newSidebarCounts = doc.querySelectorAll('.side-meta');
        var currentSidebarCounts = document.querySelectorAll('.side-meta');
        if (newSidebarCounts.length === currentSidebarCounts.length) {
            for (var i = 0; i < currentSidebarCounts.length; i++) {
                if (currentSidebarCounts[i].innerHTML !== newSidebarCounts[i].innerHTML) {
                    currentSidebarCounts[i].innerHTML = newSidebarCounts[i].innerHTML;
                }
            }
        }
    }).catch(function(){}).finally(function(){ window.__mailAutoRefreshing = false; });
}

function initLiveSearch() {
    var searchInput = document.querySelector('.search-form-top input[name="search"]');
    ...
    function performSearch() {
        ...
        if (currentBody) {
            var rows = currentBody.querySelectorAll('tr');
            rows.forEach(function(row) {
                if (row.querySelector('.empty-row')) return;
                var text = row.textContent.toLowerCase();
                var matches = terms.every(function(term) { return text.includes(term); });
    ...
    searchInput.addEventListener('input', function() {
        ...
        filter_starred_url=filter_starred_url,
        filter_important_url=filter_important_url,
