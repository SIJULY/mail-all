...
from app.utils.mail_utils import extract_code_from_body, linkify_plain_text, strip_tags_for_preview
...
def render_email_list_page(
    ...
    filter_starred_url = url_for(endpoint, **{**base_args, "page": 1, "filter": "starred", "selected_id": None})
    filter_important_url = url_for(endpoint, **{**base_args, "page": 1, "filter": "important", "selected_id": None})
        ...
        processed_emails.append(
            {
                ...
                "toggle_important_url": url_for("toggle_important", email_id=item["id"]),
            }
        )
    ...
    if selected_email:
        ...
        can_delete = (not token_view_context) if is_sent_detail else (not token_view_context and can_delete_email(selected_email, is_admin_view, token_view_context["mail"] if token_view_context else None))
        if is_sent_detail:
        ...
        selected_email_data = {
            ...
            "delete_label": "删除已发送" if is_sent_detail else ("彻底删除" if is_trash_detail else "删除邮件"),
            "delete_confirm": "确定删除这封已发送邮件吗？" if is_sent_detail else ("确定彻底删除这封邮件吗？此操作无法恢复。" if is_trash_detail else "确定删除这封邮件吗？"),
            "delete_url": url_for("delete_sent_email", sent_id=selected_email["id"]) if is_sent_detail else (url_for("permanently_delete_email", email_id=selected_email["id"]) if is_trash_detail else url_for("delete_single_email", email_id=selected_email["id"])),
...
{% elif nav_mode == 'smtp' %}
<div class="detail-page"><div class="detail-head"><h3 class="detail-title">发信设置</h3></div><div class="detail-body" style="padding:24px; background:#f8fbff; min-height:80vh;"><div class="smtp-help"><strong>说明：</strong>这里可以在线配置发信所需的 SMTP 中继参数，或使用 Resend API，保存后立即生效。<br>支持 SendGrid、Postmark、SES SMTP、Mailgun、Resend 等服务。<br>当配置完整后，左侧 <strong>撰写邮件</strong> 按钮会自动点亮。<br>Resend API 获取：注册 <a href="https://resend.com" target="_blank" style="color:#1d4ed8;">resend.com</a> 后生成 API Key 填入即可。</div>{% if sending_enabled %}<div class="smtp-status ok">当前状态：发件功能已启用</div>{% else %}<div class="smtp-status off">当前状态：发件功能未配置完整</div>{% endif %}<div class="smtp-form-card"><form method="post" action="{{ url_for('manage_smtp_settings') }}" class="smtp-form"><div class="smtp-field full"><label style="font-size:15px;color:#1e40af;border-bottom:1px solid #bfdbfe;padding-bottom:8px;margin-bottom:12px;">发信模式</label><div class="editor-switch"><label class="editor-chip"><input type="radio" name="send_mode" value="smtp" {% if not smtp_modal_data.resend_token %}checked{% endif %} onchange="document.getElementById('smtp-config-wrap').style.display='grid';document.getElementById('resend-config-wrap').style.display='none';">SMTP 模式</label><label class="editor-chip"><input type="radio" name="send_mode" value="resend" {% if smtp_modal_data.resend_token %}checked{% endif %} onchange="document.getElementById('smtp-config-wrap').style.display='none';document.getElementById('resend-config-wrap').style.display='block';">Resend API 模式</label></div></div><div id="smtp-config-wrap" style="display:{% if not smtp_modal_data.resend_token %}grid{% else %}none{% endif %};grid-template-columns:1fr 1fr;gap:12px;grid-column:1 / -1;"><div class="smtp-field"><label>SMTP Server</label><input type="text" name="smtp_server" value="{{ smtp_modal_data.server }}" placeholder="例如 smtp.sendgrid.net"></div><div class="smtp-field"><label>SMTP Port</label><input type="number" name="smtp_port" value="{{ smtp_modal_data.port }}" placeholder="例如 587"></div><div class="smtp-field"><label>SMTP Username</label><input type="text" name="smtp_username" value="{{ smtp_modal_data.username }}" placeholder="例如 apikey"></div><div class="smtp-field"><label>SMTP Password</label><input type="password" name="smtp_password" value="" placeholder="留空表示不修改已保存密码"><small>{% if smtp_modal_data.password_configured %}当前状态：已保存密钥。若不想修改，可留空后直接保存。{% else %}当前状态：尚未配置密钥，请填写后保存。{% endif %}</small></div></div><div id="resend-config-wrap" style="display:{% if smtp_modal_data.resend_token %}block{% else %}none{% endif %};grid-column:1 / -1;"><div class="smtp-field full"><label>Resend Token</label><input type="password" name="resend_token" value="" placeholder="留空表示不修改已保存 Token"><small>{% if smtp_modal_data.resend_token %}当前状态：已保存 Resend Token。若不想修改，可留空。{% else %}当前状态：尚未配置 Resend Token，请填写后保存。{% endif %}</small></div></div><div class="smtp-field full" style="margin-top:12px;"><label>默认发件邮箱</label><input type="email" name="default_sender" value="{{ smtp_modal_data.default_sender }}" placeholder="例如 no-reply@example.com" required></div><div class="smtp-field full" style="display:flex;justify-content:center;margin-top:12px;"><button type="submit" class="btn btn-primary">保存发信配置</button></div></form><form method="post" action="{{ url_for('send_test_smtp_email') }}" style="margin-top:24px;padding-top:24px;border-top:1px solid var(--line-soft);text-align:left;"><div class="smtp-field full"><label>测试发信收件人</label><div class="smtp-test-row"><input type="email" name="test_recipient" value="{{ smtp_modal_data.test_recipient }}" placeholder="例如 your@mail.com" required><button type="submit" class="btn btn-success">发送测试邮件</button></div><small>将使用当前数据库中的发信配置立即发送一封测试邮件。</small></div></form></div></div></div>
