...
def get_smtp_config() -> Dict[str, Any]:
    send_mode = get_app_setting("send_mode", "smtp").strip()
    resend_token = get_app_setting("resend_token", "")
...
def is_smtp_configured() -> bool:
    cfg = get_smtp_config()
    if not cfg["default_sender"]:
        return False
    if cfg.get("send_mode") == "resend":
        return bool(cfg.get("resend_token"))
    return bool(cfg["password"])



def send_email_via_smtp_config(to_address, subject, text_body, html_body, cfg, attachments=None):
    if not cfg["default_sender"]:
        return False, "发件功能未配置(缺少默认发件人地址)。"
        
    send_mode = cfg.get("send_mode", "smtp")
    
    if send_mode == "resend" and not cfg.get("resend_token"):
        return False, "发件功能未配置(缺少Resend Token)。"
    elif send_mode != "resend" and not cfg.get("password"):
        return False, "发件功能未配置(缺少SMTP密码/API密钥)。"
    ...
    if send_mode == "resend":
        ...
        try:
            ...
            headers = {
                "Authorization": f"Bearer {cfg['resend_token']}",
            ...
            elif not html_body.strip():
                payload["text"] = " " # Resend requires text or html
            ...
            return True, f"邮件已通过 Resend 成功发送至 {to_address}"
        except Exception as e:
            current_app.logger.error(f"通过 Resend 发送邮件失败: {e}")
            return False, f"邮件发送失败 (Resend): {e}"
